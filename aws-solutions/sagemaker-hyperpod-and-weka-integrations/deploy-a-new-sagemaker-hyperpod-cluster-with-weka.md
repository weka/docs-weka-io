...
## Deployment workflow for new SageMaker Hyperpod cluster

1. [Prepare the environment for deployment](deploy-a-new-sagemaker-hyperpod-cluster-with-weka.md#prepare-the-environment-for-deployment).
2. [Deploy WEKA Cluster using Terraform](deploy-a-new-sagemaker-hyperpod-cluster-with-weka.md#deploy-weka-cluster-using-terraform).&#x20;
3. [Create SageMaker HyperPod cluster](deploy-a-new-sagemaker-hyperpod-cluster-with-weka.md#create-sagemaker-hyperpod-cluster).&#x20;
...
1. Deploy CloudFormation template (or an equivalent) to create the prerequisites for the SageMaker HyperPod cluster.
   ...
   2. Ensure the optional parameter **"Availability zone ID to deploy the backup private subnet"** is configured with a valid entry. If the CloudFormation template has already been deployed, update the existing stack using the existing template.
...
3.  Edit the **sagemaker-hyperpod-SecurityGroup** rule created by the CloudFormation template. Add the following inbound rules to allow access from your management workstation's CIDR range:
...
7. **Continue setup**\
   Proceed with the setup by following Section 1, Step E of the [AWS SageMaker HyperPod workshop](https://catalog.workshops.aws/sagemaker-hyperpod/en-US/01-cluster/04-console).\
