...
#### Upload a snapshot using the GUI
...
#### Upload a  snapshot using the CLI
...
{% hint style="info" %}
**Note:** Only one snapshot can be uploaded to each [object store](managing-object-stores/#overview) at a time. This is so we will upload a snapshot as fast as possible, and it will not compete with other snapshots bandwidth to the same (physical) object store. There can be one snapshot upload to the local object store and one snapshot upload to the remote object store in parallel.&#x20;
{% endhint %}
...
#### Create a filesystem from a snapshot using the CLI
...
When there is a need to recover from a snapshot residing on a remote object store, there is a need to define the object-store bucket containing the snapshot as a `local` bucket. This is since normally, a remote object store has restrictions over the download, as explained in the [Managing Object Stores](managing-object-stores/#overview) section, and we would want to use a different local object-store, due to the QoS reasons explained there.&#x20;
