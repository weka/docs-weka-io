...
Using WekaFS, you can completely saturate the network. It is possible to specify several `net` devices to be used, to maximize performance.

In the WekaIO cluster, device names are generated by the order in which they appear in the command line. For example, if there are two `net` options, there will be two devices named `net0` and `net1` from the host perspective. The cluster names them by prefixing the host ID, e.g.,`host3net0`, `host3net1`. 

**For example**, the following command will allocate two cores and two physical network devices:

```text
mount -t wekafs -o num_cores=2 -o net=mlnx0,net=mlnx1 backend1/my_fs /mnt/weka
```

It is also possible to control which slots will be using which network card using different slot modifiers as `s1, s2, s2+1, s1-2, slots1+3, slot1, slots1-4` .

**For example**, by running the following command, `mlnx0` will be used by the CPU in `slot2` and `mlnx1` will be used by the CPU in `slot1`
...
**For example**, the following command will use two network devices for HA networking and allocate both devices to four frontends on the client. Note the modifier `ha` is used here, which stands for using the device on all slots.
