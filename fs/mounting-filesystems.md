...
{% hint style="info" %}
**Note:** The following information is relevant for mounting a filesystem using both the traditional method and the Stateless Clients feature. [Adding Clients](../install/bare-metal/adding-clients-bare-metal.md) \(for Bare Metal Installations\) or [Adding Clients](../install/aws/adding-clients.md) \(for AWS\).
{% endhint %}
...
Assuming the WekaIO cluster is using the backend IP of `1.2.3.4`, running this command as `root` on a client will install the agent:

`curl http://1.2.3.4:14000/dist/v1/install | sh`

On completion, the agent is installed on the client machine.

### Invoking the Mount Command <a id="configuration-process"></a>
...
The following mount options are available in addition to the mount options described in [Mounting Filesystems](mounting-filesystems.md#mount-mode-command-options). Each mount option can be passed with an individual `-o` flag to `mount:`

<table>
  <thead>
    <tr>
      <th style="text-align:left">Option Key</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="text-align:left">
        ...
        <p>For on-premises installation, if, and only if the<code>num_cores</code>option
          above is zero, a value of <code>udp</code> must be set.</p>
        <p>For<code>num_cores</code> values of 1 (as a default) or more on a non-AWS
          installation, this option should be repeated as many times as the <code>num_cores</code> option.</p>
        ...
        <p>The<code>&lt;ip&gt;</code>,<code>&lt;bits&gt;</code> and<code>&lt;gateway&gt;</code> options
          ...
          the device. The gateway itself is optional and does not have to be specified
          when no routing is required in the data network. In the cluster, device
          names are generated by the order in which they appear in the command line.
          For example, if there are two <code>--net</code> options, there will be two
          devices named <code>net0</code> and <code>net1</code>as far as the host is
          concerned. The cluster names them by prefixing the host ID, e.g.,<code>host3net0</code>, <code>host3net1</code>.</p>
      </td>
    </tr>
    <tr>
