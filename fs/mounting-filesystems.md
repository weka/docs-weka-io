...
It is also possible to control which weka compute processes will be using which network card using different modifiers as `s1, s2, s2+1, s1-2, slots1+3, slot1, slots1-4` . While s1 refers to the first compute process, s2 to the second one, and so on.

**For example**, by running the following command, `mlnx0` will be used by the second compute process and `mlnx1` will be used by the first compute process:
