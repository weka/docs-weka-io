...
| `preserved-overwritten-snapshot-name`         | String   | <p>A new name for the overwritten snapshot to preserve, thus allowing the IO operations continuity to the filesystem.<br>If not specified, the original snapshot or active filesystem is overwritten, and IO operations to an existing filesystem might fail.</p> |                               | No            |                                                                                                                                                                                                     |
| `preserved-overwritten-snapshot-access-point` | String   | A directory that serves as the access point for the preserved overwritten snapshot.                                                                                                                                                                               |                               | No            | If the `preserved-overwritten-snapshot-name` parameter is specified, but the `preserved-overwritten-snapshot-access-point`parameter is not, it is created automatically based on the snapshot name. |
