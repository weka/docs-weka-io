...
2. From the `wekahome_offline` directory, run `./update_config.sh`.\
   (If you update the Local Weka Home release, skip the `config.yaml` file configuration step.)
...
Once the Local Weka Home is deployed, you can set it to send alerts by email, SNMP, or PagerDuty. See the [Set the Local Weka Home to send alerts](local-weka-home-deployment.md#set-the-local-weka-home-to-send-alerts) section.
{% endhint %}
...
&#x20;   or **Completed**.\
&#x20;   (To wait for the pods' statuses, run `watch kubectl get pods`.)
