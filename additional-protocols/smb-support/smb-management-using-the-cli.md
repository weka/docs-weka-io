...
* [Control SMB access based on host IP/name](smb-management-using-the-cli.md#control-smb-access-based-on-ip)
...
| `idmap-backend`                  | String                       | The Id mapping method to use.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | <p>SMB-W: <code>rfc2307</code><br><code></code><br><code></code>Legacy SMB: <code>rfc2307</code> or <code>rid</code></p>                                                                                                  | No            | `rfc2307`                       |
...
| **Name**   | **Type** | **Value**                                                     | **Limitations**                  | **Mandatory** | **Default** |
| ---------- | -------- | ------------------------------------------------------------- | -------------------------------- | ------------- | ----------- |
| `username` | String   | Name of a user with permissions to add a server to the domain | Must be a valid name (ASCII)     | Yes           |             |
...
`weka smb cluster containers add [--container-ids container-ids]...`

`weka smb cluster containers remove [--container-ids container-ids]...`
...
| **Name**        | **Type**                | **Value**                                                                                             | **Limitations**             | **Mandatory** | **Default** |
| --------------- | ----------------------- | ----------------------------------------------------------------------------------------------------- | --------------------------- | ------------- | ----------- |
| `container-ids` | Comma-separated strings | List of 3-8 Weka system servers to participate in the SMB cluster, based on the container IDs in Weka | Must be valid container IDs | Yes           |             |

## Configure trusted domains <a href="#configure-trusted-domains" id="configure-trusted-domains"></a>

{% hint style="info" %}
**Note:** SMB-W does not support trusted domains.
{% endhint %}
...
| `allow-guest-access`    | String                            |  Allows connecting to the SMB service without a password. Permissions are as the `nobody` user account permissions.                                                                                                                                                                                                                                                                                                                                                                                                                                            | <p>Legacy SMB: <code>on</code> or <code>off</code><br><code></code>SMB-W: not supported</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | No            | `off`             |
...
{% hint style="info" %}
**Note:** SMB-W does not support share update.
{% endhint %}
...
{% hint style="info" %}
**Note:** SMB-W does not support share user-lists.
{% endhint %}
...
{% hint style="info" %}
**Note:** SMB-W does not support access based on hosts' IP/name.
{% endhint %}

**Command:** `weka smb cluster host-access show`

&#x20;         `weka smb share host-access show`
