...
For WEKA S3, use the S3 access key and secret key generated by WEKA. The system creates these credentials automatically when an administrator attaches an S3 policy to a user, or when an administrator or user runs `weka s3 user keys-generate`.

Do not use the WEKA account username and password in the `aws_access_key_id` and `aws_secret_access_key` fields.
...
Use the WEKA-generated S3 access key and secret key in the boto3 session:
