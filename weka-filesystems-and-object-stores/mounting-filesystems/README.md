...
* [Read cache mount mode](../../weka-system-overview/weka-client-and-mount-modes.md#read-cache-mount-mode)
* [Write cache mount mode](../../weka-system-overview/weka-client-and-mount-modes.md#write-cache-mount-mode-default)
...
To restrict mounting to only WEKA authenticated users, set the `--auth-required` flag to `yes` for the filesystem. For more information, refer to [Obtain authentication tokens](../../security/obtain-authentication-tokens.md).
...
{% hint style="info" %}
When running in AWS, the instance IAM role must provide permissions to several AWS APIs (see the [Modules overview](../../planning-and-installation/aws/detailed-deployment-tutorial-weka-on-aws-using-terraform.md#modules-overview) section).
{% endhint %}

{% hint style="info" %}
Memory allocation for a client is predefined. To change the memory allocation, contact the [Customer Success Team](../../support/getting-support-for-your-weka-system.md#open-a-support-case).
{% endhint %}
